...
for (let btn of favBtns) {
    btn.addEventListener("click", function (e) {
        ...
        let isAlreadyFav = favItems.some((item) => item.id === selectedItem.id);

        if (!isAlreadyFav) {
            favItems.push(selectedItem);
            ...
            Swal.fire({
                icon: 'success',
                title: `${selectedItem.title} added to favorites`,
            });
        } else {
            Swal.fire({
                icon: 'info',
                title: 'Item is already in favorites',
            });
        }
    ...
    for (let btn of favBtns) {
        btn.addEventListener("click", function (e) {
            ...
            if (hasAdded.classList.contains("fa-regular")) {
                ...
                let selectedItem = data[btn.name];
                let favItems = JSON.parse(localStorage.getItem("favItems")) || [];
                let removeIndex = favItems.findIndex(item => item.id === selectedItem.id);
                if (removeIndex !== -1) {
                    ...
                    Swal.fire({
                        icon: 'success',
                        title: `${selectedItem.title} removed from favorites`,
                    });
                }
            }
        });
